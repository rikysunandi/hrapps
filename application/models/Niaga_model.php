...
class Niaga_model extends CI_Model {
	...
	function get_tb_lead_measure($unitpln,$unitpln2,$tglpilih,$tglpilih2) {		
		if ($unitpln == '0') {
		...
		select a.*,b.nama namaap,c.nama namaup from tb_lead_measure a,tb_unitap b,tb_unitup c
			where $q  and a.bidang='NIAGA'
		and a.unitap=c.unitap and a.unitup = c.unitup and a.unitap = b.unitap
		order by a.unitap,a.unitup,a.id_lm,a.tanggal asc
		")->result();
    }
	
	function get_tb_rekap_lead_measure($unitpln,$unitpln2,$tglpilih) {		
		if ($unitpln == '0') {
...
on a.unitup = b.unitup and $q  and b.bidang='NIAGA'
...
order by unitap asc,unitup asc")->result();
	...
	public function insert_niaga_kogol($data){	
	...
	public function insert_niaga_lembar($data){	
     ...
     //	$this->db->query("DROP temporary table dashboard.tb_tunggakan_kogol_temp")->result();
		...
		if($numrow > 0){
			/*	if($unitups) {
				...
				if ($lembar <> '') {
				$this->db->query("CALL proc_niaga_lembar('$unitup','$lembar','$jml_plg','$jml_lbr','$rpptl','$rpbpju','$rpppn','$rpmat','$rplain','$rptag','$rpbk','$keterangan','$tgl_insert')");
				}
	...
	public function insert_niaga_lembar2($data){	
     ...
     //	$this->db->query("DROP temporary table dashboard.tb_tunggakan_kogol_temp")->result();
		...
		if($numrow > 0){
			/*	if($unitups) {
				...
				if ($kd_rayon <> '') {
				$this->db->query("CALL proc_niaga_lembar2('$kd_rayon','$nama_rayon','$lbr_awal1','$lbr_awal2','$lbr_awal3','$lbr_awal4',
