...
class Niaga extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model('Niaga_model');
	...
	public function data_kogol() {  
		...
		$this->template->display('niaga/tb_niaga_data_kogol',$data);
    }
	
	public function data_lembar() {    
		$data['data'] = $this->General_model->getUnitAp_by();
	
		$this->template->display('niaga/tb_niaga_data_lembar2',$data);
    }
	...
	public function lead_measure() {    
		...
		$data['wig'] = $this->db->query("select * from tb_master_wig where bidang='NIAGA' order by id_wig asc")->result();
		$this->template->display('niaga/tb_niaga_lead_measure',$data);
		
		//$data['getunitap']=$this->General_model->getUnitPln();
        //$this->template->display('niaga/tb_niaga_lead_measure',$data);
		
        //$this->template->display('niaga/tb_niaga_lead_measure');
    }
	...
	public function rekap_lead_measure() {    
		...
		$data['wig'] = $this->db->query("select * from tb_master_wig where bidang='DISTRIBUSI' order by id_wig asc")->result();
		 $this->template->display('niaga/tb_niaga_rekap_lm',$data);
	...
	public function upload_kogol() {    
        $this->template->display('niaga/tb_niaga_upload_kogol');
    }
	
	public function upload_lembar() {    
        $this->template->display('niaga/tb_niaga_upload_lembar2');
    }
	...
	function view_lead_measure(){ 
        ...
        $no=1;               
        $transaksi_data = $this->Niaga_model->get_tb_lead_measure($unitpln,$unitpln2,$tglpilih,$tglpilih2);
	...
	function view_rekap_lead_measure(){ 
        ...
        $no=1;               
        $transaksi_data = $this->Niaga_model->get_tb_rekap_lead_measure($unitpln,$unitpln2,$tglpilih);
            foreach ($transaksi_data as $transaksi) {
				$edit = '<a class="btn btn-xs btn-success" href="javascript:void(0)" title="update" onclick="edit_lm('."'".$transaksi->id."'".')"><i class="glyphicon glyphicon-edit"></i>update</a>';
	...
	function view_lm_id($id){ 
		...
		$no=1;               
        $transaksi_data = $this->Niaga_model->get_lm_id($id);
            foreach ($transaksi_data as $transaksi) {  
	...
	function tambah_lm(){ 
		...
		$hasil = $this->Niaga_model->tambah_lm($id_wig,$id_lm,$satuan,$target,$keterangan,$unitap,$unitup,$bidang);		
		$a = 'Tambah LM ||'.$tgl_tagihan;
		
		$this->session->set_flashdata('msg', $a);
		//log_message('error',$a);
		
		redirect("niaga/lead_measure");
    }  
	...
	function view_data_kogol(){ 
        ...
        $no=1;               
        $transaksi_data = $this->Niaga_model->get_tb_data_kogol($unitpln,$unitpln2,$tglpilih);
            foreach ($transaksi_data as $transaksi) {
			$query[] = array(
                'no'=>$no++,
                'unitupi'=>$transaksi->unitupi,
	...
	function view_data_lembar(){ 
        ...
        $no=1;               
        $transaksi_data = $this->Niaga_model->get_tb_data_lembar($unitpln,$tglpilih);
            foreach ($transaksi_data as $transaksi) {
			$query[] = array(
                'no'=>$no++,
                'unitupi'=>$transaksi->unitupi,
	...
	function view_data_lembar2(){ 
        ...
        $no=1;               
        $transaksi_data = $this->Niaga_model->get_tb_data_lembar2($unitpln,$unitpln2,$tglpilih);
            foreach ($transaksi_data as $transaksi) {
	...
	public function niaga_upload_kogol(){
		...
		if(isset($_POST['preview'])){ // Jika user menekan tombol Preview pada form
			...
			$upload = $this->Niaga_model->upload_file($this->filename);
		...
		$this->template->display('niaga/tb_niaga_upload_kogol', $data);
		//$this->load->view('tracking/form', $data);
	}
	
	public function niaga_upload_lembar(){
		...
		//$this->load->view('tracking/tracking_upload', $data);
		$this->template->display('niaga/tb_niaga_upload_lembar2', $data);
		//$this->load->view('tracking/form', $data);
	}
	
	public function niaga_import_lembar(){
		...
		$this->Niaga_model->insert_niaga_lembar($data);
		
		//$this->db->query("CALL proc_niaga_lembar('$rand')");
		
		redirect("niaga/data_lembar"); // Redirect ke halaman awal (ke controller siswa fungsi index)
	}
	
	public function niaga_import_lembar2(){
		...
		// Panggil fungsi insert_multiple yg telah kita buat sebelumnya di model
		//$this->db->set('tgl_insert', 'current_date()', FALSE);
		$this->Niaga_model->insert_niaga_lembar2($data);
	...
	public function niaga_import(){
		...
		// Panggil fungsi insert_multiple yg telah kita buat sebelumnya di model
		//$this->db->set('tgl_insert', 'current_date()', FALSE);
		$this->Niaga_model->insert_niaga_kogol($data);
		
		$this->db->query("CALL proc_niaga_kogol('$rand')");
		
		redirect("niaga/data_kogol"); // Redirect ke halaman awal (ke controller siswa fungsi index)
	}
